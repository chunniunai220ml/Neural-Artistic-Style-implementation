...
def reshapeBackToGrid(inputM, sideLength):
    sideLength = int(math.sqrt(inputM.shape[1]))
    D = np.zeros((sideLength, sideLength, inputM.shape[0]))
    for k in range(inputM.shape[0]):
        for i in range(int(sideLength)):
            for j in range(int(sideLength)):
                D[i, j, k] = inputM[k, sideLength * i + j]    
